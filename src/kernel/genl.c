...
static void set_msg_data(struct genl_msg_data *msg_data, int type, void *data,
			 int len)
{
	msg_data->atype = type;
	msg_data->data = data;
...
static void extract_ti_mfa_attrs(const struct genl_info *info,
				 struct ti_mfa_param *a)
{
	a->command = (char *)extract_nl_attr(info, TI_MFA_A_COMMAND);
	a->link_source =
		(struct mac *)extract_nl_attr(info, TI_MFA_A_LINK_SOURCE);
	a->link_dest = (struct mac *)extract_nl_attr(info, TI_MFA_A_LINK_DEST);

	a->dest = (struct mpls_dest *)extract_nl_attr(info,
						      TI_MFA_A_BACKUP_LABEL);
	a->backup_dev_name =
		(char *)extract_nl_attr(info, TI_MFA_A_BACKUP_DEV_NAME);
...
{
	if (ti_mfa_attr->link_source != NULL)
		pr_debug("Source: %pM\n", ti_mfa_attr->link_source->oct);
	if (ti_mfa_attr->link_dest != NULL)
		pr_debug("Dest: %pM\n", ti_mfa_attr->link_dest->oct);

	if (ti_mfa_attr->dest != NULL)
		pr_debug("Label: %u\n", ti_mfa_attr->dest->label);
	if (ti_mfa_attr->backup_dev_name != NULL)
		pr_debug("NetDev: %s\n", ti_mfa_attr->backup_dev_name);

	if (ti_mfa_attr->net_ns != NULL)
		pr_debug("NetNS PID: %d\n", ti_mfa_attr->net_ns->pid);
}
			 ...
			 struct genl_msg_data *msg_data)
{
	...
	skb_head = genlmsg_put(skb, 0, info->snd_seq + 1, &ti_mfa_genl_family,
			       0, info->genlhdr->cmd);
	if (skb_head == NULL) {
	...
	for (i = 0; i < n_data; i++) {
		int ret;
		if ((ret = nla_put(skb, msg_data[i].atype, msg_data[i].len,
				   msg_data[i].data)) < 0) {
			pr_err("send_response - unable to put attribute %d for elem %d/%d: %d",
			       msg_data[i].atype, i, n_data, ret);
			return -1;
		}
	}
	...
	if (genlmsg_unicast(genl_info_net(info), skb, info->snd_portid) != 0) {
		pr_err("send_response - unable to send response - info->snd_portid = %u",
		       info->snd_portid);
		return -1;
	}
...
static int add_backup_route(struct ti_mfa_param attr, struct genl_info *info)
{
	...
	set_msg_data(data, TI_MFA_A_RESPONSE, err_str[ret],
		     strlen(err_str[ret]));
...
static int show_all_routes(struct ti_mfa_param attr, struct genl_info *info)
{
	...
	case TI_MFA_RT_OK:
		if (strlen(message) > len)
			set_msg_data(data, TI_MFA_A_RESPONSE, RESPONSE_ER,
				     strlen(RESPONSE_ER));
		else
			set_msg_data(data, TI_MFA_A_RESPONSE_LST, message,
				     strlen(message));
		break;
	case TI_MFA_RT_ROUTING_TABLE_EMPTY:;
		ret = 2;
		fallthrough;
	default:
		set_msg_data(data, TI_MFA_A_RESPONSE_LST, err_str[ret],
			     strlen(err_str[ret]));
	}
