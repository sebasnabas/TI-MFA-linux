...
ti_mfa_protocol.fields = {
  f_node_source, f_link_source, f_link_dest, f_bos
}

function ti_mfa_protocol.dissector(buffer, pinfo, tree) length = buffer:len()
  ...
  do
    ...
    subtree:add(f_link_dest, buffer(offset, 6))
    subtree:add(f_link_source, buffer(offset + 6, 6))
    subtree:add(f_bos, buffer(offset + 12, 1))

    is_bos = buffer(offset + 12, 1):uint() == 1
